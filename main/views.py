...
def home(request):
    if request.user.is_authenticated:
        # Check if user explicitly wants to see the main site (e.g. from Home/Menu links)
    ...
    if request.user.is_authenticated and request.user.role == 'delivery_person':
        try:
            ...
            rider_tasks_count = Order.objects.filter(delivery_person=rider).exclude(status=Order.Status.DELIVERED).count()
...
def my_orders(request):
    ...
    if request.user.role == CustomUser.Roles.DELIVERY_PERSON:
        try:
            rider = DeliveryPerson.objects.get(user=request.user)
            deliveries = Order.objects.filter(delivery_person=rider).exclude(status=Order.Status.DELIVERED).order_by('-created_at')
        except DeliveryPerson.DoesNotExist:
            pass
...
def rider_dashboard(request):
    ...
    try:
        rider = DeliveryPerson.objects.get(user=request.user)
        assigned_orders = Order.objects.filter(delivery_person=rider).select_related('shipping_address', 'payment').order_by('-created_at')
    except DeliveryPerson.DoesNotExist:
        assigned_orders = []
